...
class CRFTemplateView(GenericViewSet,
    ...
    def update(self, requests, pk=None):
        ...
        if patient_chart_reviews:
            message = "Can not edit crf template '{}' which is involved in patient curation".format(instance.name)
            res = {'errors': message}
            return Response(res)
    ...
    def destroy(self, request, pk=None):
        ...
        patient_chart_reviews = PatientChartReview.objects.filter(
            ...
            crf_template_id=instance.id).distinct('crf_template_id')
        if patient_chart_reviews:
            message = "Can not delete crf template '{}' which is involved in patient curation".format(instance.name)
            res = {'errors': {}}
